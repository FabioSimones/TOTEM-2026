...
- `src/components/ui/Modal.test.tsx` (TASK-110) — não renderiza nada fechado; renderiza título/conteúdo com `role="dialog"`/`aria-modal` quando aberto; fecha pelo botão X, por Escape, e pelo backdrop só quando `fecharAoClicarBackdrop` é `true` (o padrão é `false`).
- `src/pages/admin/AdminRestaurantesPage.test.tsx` (TASK-110) — página representativa do novo padrão modal: lista aparece sem formulário permanente; "Novo restaurante" abre o modal vazio e "Cancelar" fecha; cadastrar fecha o modal e atualiza a lista; "Editar" abre o modal já preenchido com os dados do restaurante. As demais páginas (`Dispositivos`/`Categorias`/`Produtos`/`Usuários`) seguem o mesmo padrão de código, mas não ganharam teste próprio nesta task (ver "Recomendações" em `docs/roadmap-pos-mvp.md`).
- `src/pages/caixa/CaixaHomePage.test.tsx`/`src/pages/cozinha/CozinhaHomePage.test.tsx` (TASK-111, ampliado na TASK-112) — sem dispositivo mostra o card "Ativar este dispositivo" (e não chama nenhum serviço); dispositivo de outro tipo mostra "Dispositivo incompatível" citando o tipo atual; "Trocar dispositivo" pede confirmação (`window.confirm` mockado) — cancelar preserva a sessão, confirmar limpa dispositivo+operador e navega para `/ativar-dispositivo`; sem operador identificado mostra só o login centralizado (com "Trocar dispositivo" como ação secundária) e nunca chama `listarPendencias`/`listarPedidosCozinha`; login do operador dispara a busca e mostra a lista; "Trocar operador" oculta a lista imediatamente e volta ao login.
- `src/services/tokenStorage.test.ts` ganhou um caso para `limparSessaoOperacionalCompleta` (TASK-112): limpa tokens/dispositivo/operador, mas nunca `totem.usuario`.
...
- `e2e/admin-login.spec.ts` — login central (formulário, clique, navegação para `/admin`, `localStorage` real) com sucesso e com credenciais inválidas (mensagem de erro, permanece em `/login`); TASK-116: confirma também que a rota antiga `/admin/login` redireciona para `/login`.
- `e2e/login-centralizado.spec.ts` (TASK-116, novo) — homologação do login centralizado além do caminho feliz: `ADMIN_RESTAURANTE` também vai para `/admin`; rota administrativa sem sessão redireciona para `/login` e, após autenticar, volta para a rota originalmente solicitada (`ProtectedRoute`/`location.state.from`); `OPERADOR_CAIXA`/`OPERADOR_COZINHA` sem dispositivo compatível ativado neste navegador permanecem no login com orientação para `/ativar-dispositivo` (nunca liberados só com o token de usuário); operador com dispositivo compatível é liberado para `/caixa`; Totem com dispositivo incompatível (ex.: um Caixa ativado) nunca chega a exibir o cardápio; logout administrativo não remove um dispositivo já ativado no mesmo navegador e vice-versa (`trocar dispositivo` não remove a sessão administrativa); com operador identificado, `Authorization` carrega o token do dispositivo e `X-Operador-Token` o do operador, nunca invertido. Proteção contra open redirect (`location.state.from` externo) é coberta por teste unitário (`src/pages/auth/LoginPage.test.tsx`), não e2e — mais confiável para simular o `history.state` do React Router do que forjar isso no navegador real.
...
- `e2e/fluxo-operacional-mock.spec.ts` — cada terminal testado isoladamente: Totem carrega o cardápio e "Adicionar" habilita o resumo do pedido; Caixa e Cozinha (TASK-111: atualizado para logar o operador antes de checar a lista, já que sem operador nenhum pedido aparece mais) confirmam que a lista só aparece depois do login do operador, com o botão de ação sugerida/avanço de status visível em seguida. Não encadeia um pedido entre os três (ver "Não coberto" acima).
- `e2e/ativacao-dispositivo.spec.ts` (TASK-112, novo) — `/caixa` sem dispositivo mostra o card de ativação e o botão leva a `/ativar-dispositivo`; ativação mockada de um dispositivo CAIXA redireciona de volta para `/caixa` e o login do operador continua funcionando normalmente em seguida; "Trocar dispositivo" (com operador já identificado) limpa dispositivo+operador do `localStorage` e volta para `/ativar-dispositivo`.
- `e2e/acessibilidade.spec.ts` (TASK-113, ampliado na TASK-114) — confirma **no navegador real**, não só no CSS: o botão de quantidade do carrinho do Totem tem `boundingBox()` ≥44×44px e continua incrementando; o `ThemeToggle` tem ≥44×44px e continua alternando `data-theme`; o botão de fechar do modal administrativo tem ≥44×44px e continua fechando o modal; `Tab` no login administrativo produz um elemento focado com `outline-style`/`outline-width` visíveis (não `none`/`0px`). TASK-114: "Trocar operador" (agora `Button variant="secondary"`) mantém ≥44px de altura; "Cancelar pedido" (`variant="danger"`) e as ações de card de Restaurantes ficam visíveis em mobile (375px) sem gerar `scrollWidth` maior que o viewport — este último teste pegou uma regressão real de padding introduzida durante a própria implementação, corrigida antes de finalizar a task.
- `src/components/ui/Button.test.tsx` (TASK-114, novo) — variante padrão sem classe extra; `variant="secondary"`/`"danger"` aplicam a classe correspondente; `fullWidth` aplica `ui-button--full`; `loading` mostra "Aguarde...", desabilita e marca `aria-busy="true"`; sem `loading` não tem `aria-busy`; `disabled` funciona sem `loading`; `onClick` dispara quando habilitado; `className` extra é preservado junto das classes do componente.
- `e2e/formularios-validacao.spec.ts` (TASK-115, novo) — validação inline mockada, não exaustiva: Restaurante (submit vazio → erro inline + foco no campo → preencher e salvar fecha o modal), Produto (submit sem obrigatórios → erros inline em nome/preço → modal permanece aberto → primeiro campo focado), Usuário (e-mail inválido → mensagem acessível → corrigir → cadastro prossegue e fecha o modal).
- `src/components/ui/{FieldError,Input}.test.tsx`, `src/utils/{validacaoFormulario,apiFieldErrors}.test.ts`, `src/components/admin/**/{RestauranteForm,ProdutoForm,UsuarioForm}.test.tsx` (TASK-115, novos) — cobrem o padrão de validação inline em nível de componente: submit vazio gera erro por campo sem chamar o serviço, corrigir o campo remove o erro, foco vai para o primeiro campo inválido, erro de campo vindo da API (`errosCampoApi`) aparece inline sem duplicar a mensagem global. `AdminRestaurantesPage.test.tsx` ampliado com os casos "não fecha o modal em erro local/de API" e "sem erro residual ao reabrir".
- `e2e/redesign-login.spec.ts` (TASK-117, novo) — homologação do redesign do login: `/` redireciona para `/ativar-dispositivo`; `/login` continua acessível direto, com o novo título "Acesse sua conta" e sem link de recuperação de senha; navegação cruzada entre `/login` ↔ `/ativar-dispositivo` ("Ativar um dispositivo"/"Entrar como usuário"); login SUPER_ADMIN continua indo para `/admin`; mobile (375px) sem scroll horizontal, com `ThemeToggle` ≥44×44px funcional; desktop (1440px) sem scroll horizontal, com os 3 ícones decorativos (`FoodIcons`) visíveis; `prefers-reduced-motion` emulado confirma `animation-duration` reduzido a ~0 nos ícones, e sem essa preferência a animação declarada continua ativa. `e2e/admin-login.spec.ts` atualizado para o novo título do heading ("Acesse sua conta", antes "Login").
- `src/routes/AppRoutes.test.tsx` (TASK-117, novo; ampliado na TASK-118) — nível de unidade/integração para o roteamento: `/` renderiza `AtivarDispositivoPage`, `/login` renderiza `LoginPage`, `/admin/login` continua redirecionando para `/login`. TASK-118: `/admin` (índice) renderiza `AdminDashboardPage` dentro do novo `AdminLayout`, `/admin/restaurantes` e `/admin/usuarios` continuam restritos por `RoleGuard` para `ADMIN_RESTAURANTE`, `/admin/dashboard` (rota antiga) redireciona para `/admin`.
- `src/pages/auth/LoginPage.test.tsx` (TASK-117, ampliado) — confirma um único `<h1>` ("Acesse sua conta"), ausência de qualquer texto/link "Esqueceu a senha?", link para `/ativar-dispositivo`, conteúdo institucional renderizado (marca + recursos) sem citar perfis técnicos (`SUPER_ADMIN` etc.) na UI, e que os SVGs decorativos (`FoodIcons`) têm `aria-hidden`/`focusable="false"` e não aparecem como `button`/link navegável.
- `e2e/admin-layout.spec.ts` (TASK-118, novo) — homologação do novo layout administrativo (sidebar + topbar) via login real e via sessão pré-semeada (`seedAdminSession`): SUPER_ADMIN vê a sidebar expandida com nome/perfil e os 7 links permitidos; recolher/expandir a sidebar altera sua largura (`expect.poll` aguardando a transição CSS) e o rótulo dos links permanece acessível mesmo recolhido; navegar por um item com a sidebar recolhida funciona; o `ThemeToggle` da topbar alterna `data-theme`; logout leva a `/login`. Drawer mobile (375px): fechado por padrão, hambúrguer abre com backdrop visível, clique no backdrop fecha, `Escape` fecha, selecionar uma rota fecha o menu, sem scroll horizontal no dashboard. `ADMIN_RESTAURANTE`: link "Restaurantes" não aparece na sidebar; acesso direto a `/admin/restaurantes` continua bloqueado pelo `RoleGuard` mesmo vindo do redirecionamento de login (mesmo padrão de navegação única já estável em `login-centralizado.spec.ts`, sem `page.goto` duplo).
- `src/hooks/useAdminSidebarCollapsed.test.ts`, `src/components/layout/{AdminSidebar,AdminTopbar,AdminLayout}.test.tsx`, `src/pages/admin/AdminDashboardPage.test.tsx` (TASK-118, novos) — nível de componente: persistência do estado recolhido/expandido em `localStorage` (com fallback seguro para valores inválidos); sidebar renderiza só os links permitidos ao perfil (via `itensVisiveisParaPerfil`); topbar deriva título/descrição da rota atual (via `encontrarItemPorRota`) e mostra o rótulo amigável do perfil (nunca o enum cru); `AdminLayout` fecha o drawer mobile e devolve o foco ao botão de hambúrguer ao pressionar `Escape`; `AdminDashboardPage` carrega `GET /api/admin/dashboard`, trata erro 401 (chama `logout`)/403/erro genérico e formata os indicadores (incluindo `valorPagoHoje` em BRL).
- `e2e/painel-operacional.spec.ts` (TASK-119, novo) — homologação do redesign dos painéis operacionais (Caixa/Cozinha): a `OperationalTopbar` mostra módulo/dispositivo/operador com perfil amigável (nunca o enum técnico); a ação sugerida (ex.: "Confirmar dinheiro") executa e atualiza a lista; "Trocar operador" (na topbar) remove os pedidos da tela imediatamente e preserva o dispositivo; "Trocar dispositivo" com confirmação cancelada preserva a sessão, confirmada limpa dispositivo+operador e navega para `/ativar-dispositivo`; `ThemeToggle` funciona a partir da nova topbar; ação operacional tem ≥48px de altura; mobile (375px) sem scroll horizontal; sessão expirada (401 na listagem) remove os pedidos e volta ao card de ativação; navegação por Tab alcança as ações da topbar com foco visível. Cozinha: fila mostra o tempo decorrido de cada pedido, transição de status ("Iniciar preparo") atualiza a fila, "Trocar operador" esvazia a fila, perfil "Operador de cozinha" exibido corretamente. `e2e/operador-painel.spec.ts` e `e2e/ativacao-dispositivo.spec.ts` ajustados: a identidade do operador identificado (antes "Operador: {nome}", renderizada dentro do fluxo de conteúdo) agora aparece na topbar, sem o prefixo "Operador:".
- `src/components/layout/{OperationalTopbar,OperationalLayout}.test.tsx`, `src/components/ui/{PedidoStatusBadge,OperationalEmptyState}.test.tsx`, `src/components/{caixa/PedidoPendenteCard,cozinha/PedidoCozinhaCard}.test.tsx` (TASK-119, novos) — nível de componente: topbar mostra módulo/dispositivo/operador (com perfil amigável, nunca `OPERADOR_CAIXA`/`OPERADOR_COZINHA` cru) e aciona `onTrocarOperador`/`onTrocarDispositivo`; layout renderiza `banner`+`main` sem nenhuma `navigation` (confirma ausência de `AdminSidebar`); badge de status mostra texto, não enum; estado vazio/erro/carregando padronizado (`aria-busy` no carregamento, botão "Tentar novamente" só quando há callback); cards mostram status/valor/ação (Caixa) e status/tempo decorrido/ação (Cozinha), com `window.confirm` antes de cada ação e `disabled`/`aria-busy` durante execução. `src/components/operador/OperadorPainel.test.tsx` reduzido ao formulário de identificação (a exibição do operador identificado, antes testada aqui, migrou para `OperationalTopbar.test.tsx`). `src/utils/dateTime.test.ts` ampliado com `formatarTempoDecorrido` (fallback, minutos, horas exatas, horas+minutos, nunca negativo).
- `e2e/usuario-card-layout.spec.ts` (TASK-119.1, novo) — corrige a sobreposição de e-mail/perfil/restaurante nos cards de `/admin/usuarios`: confirma, no navegador real, que os 3 campos ficam em blocos separados (rótulo + valor, um por linha); os 4 perfis mostram o rótulo amigável, nunca o enum técnico; e-mail e nome de restaurante longos ficam totalmente visíveis, sem cortar; nenhum card gera `scrollWidth > clientWidth` em desktop (1440px) nem em mobile (375px); botões de ação (Editar/Desativar/Alterar senha) continuam presentes. `src/components/admin/usuarios/UsuarioCard.test.tsx` (novo) cobre o mesmo em nível de componente, incluindo o caso sem restaurante (exibe "—", mantém a linha "Restaurante").
- **TASK-119.2** (integração do login do operador ao layout operacional) — corrige o intervalo "dispositivo pronto, operador ausente" em `/caixa`/`/cozinha`, que ainda usava a casca visual antiga (`AppLayout`/`ModuleHeader`) mesmo depois da TASK-119 introduzir o layout operacional novo. `e2e/painel-operacional.spec.ts` ganhou o describe `"Integração do login do operador ao layout operacional (TASK-119.2)"`: ativar um dispositivo COZINHA mostra a `OperationalTopbar` completa (módulo, dispositivo, `ThemeToggle`, "Trocar dispositivo") imediatamente, sem nenhum pedido nem "Trocar operador" visível antes do login; `ThemeToggle` funciona antes do login; erro de credencial inválida mantém a topbar e não expõe pedidos; campo de e-mail recebe foco automático; mobile (375px) sem operador não gera scroll horizontal. `e2e/ativacao-dispositivo.spec.ts`, `operador-painel.spec.ts`, `fluxo-operacional-mock.spec.ts`, `login-centralizado.spec.ts` ajustados: a asserção antiga (`getByText(/Operador não identificado/)`, texto que existia dentro do `OperadorPainel` isolado) foi substituída pela verificação do novo `<h1>` ("Identifique-se para acessar o Caixa"/"...a Cozinha") dentro do layout operacional. `src/components/layout/{OperationalTopbar,OperationalLayout}.test.tsx` ganharam o estado sem operador (topbar/layout montam sem avatar/perfil/"Trocar operador", mas com dispositivo/ThemeToggle/"Trocar dispositivo"); `src/components/operador/OperadorPainel.test.tsx` reescrito para a nova API (`titulo`/`descricao`, sem `acaoTrocarDispositivo`) incluindo foco automático, erro preservando os campos, e loading desabilitando os inputs; `src/pages/{caixa/CaixaHomePage,cozinha/CozinhaHomePage}.test.tsx` ganharam casos confirmando a topbar única (sem duplicação de "Trocar dispositivo") durante toda a transição sem→com operador.
...
| CI | Roda (`Frontend E2E (Playwright)`), check recomendado | Roda desde a TASK-106 (`Frontend E2E Integrado (Backend real)`), check recomendado — ver seção "CI" abaixo |
...
Desde a TASK-106, esta suíte também roda no GitHub Actions: job `Frontend E2E Integrado (Backend real)` em `.github/workflows/ci.yml`, com `needs: [frontend, frontend-e2e, backend-postgres-it]` (só roda depois que o básico do frontend, o E2E mockado e a suíte real do backend contra Testcontainers já passaram — é o job mais caro do workflow). O job:

1. Sobe PostgreSQL via `services:` do próprio GitHub Actions (imagem `postgres:16`, efêmero, destruído ao fim do job — nenhum Docker Compose novo).
2. Sobe o backend real em background (`nohup mvn -B spring-boot:run & disown`) com as mesmas variáveis descritas nos pré-requisitos acima (valores descartáveis, direto no `env:` do job — nunca segredos reais nem produção).
3. Aguarda `/actuator/health` responder antes de prosseguir (mesmo mecanismo do `global-setup.ts` local, só que como um step de espera explícito no workflow).
4. `npm ci` → instala o Chromium → `npm run e2e:integrado`.
5. Em falha, publica um artifact (`playwright-report-integrado`) com o relatório HTML, `test-results-integrado/` (traces/screenshots) **e** o log do backend (`backend-e2e-integrado.log`) — útil para diferenciar "backend não subiu"/"erro de negócio real" de "teste flakou".

**Validado localmente antes do push**: reproduzi o mesmo padrão do workflow (PostgreSQL na porta usada pelo job, backend subido via `nohup mvn -B spring-boot:run & disown`, espera por `/actuator/health`) e `npm run e2e:integrado` → 2/2 passou. **Não é check obrigatório** — ver `docs/ci-branch-protection.md` para a justificativa (job novo, mais superfície de instabilidade transitória que o E2E mockado, precisa de vários runs reais antes de virar obrigatório).

### Limitações e o que ainda não está coberto

- Dados de teste se acumulam no banco a cada execução (sem cleanup automatizado) — aceitável para um banco descartável local/CI, não deve ser usado contra um banco compartilhado.
...
- Nenhum fluxo Totem→Caixa→Cozinha encadeado com o **mesmo** pedido numa única execução (os dois specs preparam pedidos independentes).
- Não substitui a suíte mockada (que continua cobrindo mais fluxos/telas e é o check mais barato/estável do CI) — as suítes são complementares.
...
│   ├── ui/         # Button, Input, ErrorMessage, ThemeToggle, Modal (TASK-110) — componentes mínimos reutilizáveis
│   ├── dispositivo/ # DispositivoAcessoCard (TASK-112) — card centralizado de ativar/trocar dispositivo
│   └── totem/      # CategoriaCardapioSection, ProdutoCard — específicos da tela de cardápio
├── contexts/       # ThemeContext.tsx — estado do tema
├── hooks/          # useTheme.ts, useCart.ts, useDispositivoOperacional.ts (TASK-112)
...
| `/caixa` | `CaixaHomePage` | **Real** — lista de pendências, confirmar dinheiro, enviar à cozinha, cancelar e marcar retirada do dispositivo CAIXA (ciclo completo). Desde a TASK-111, exige login de `OPERADOR_CAIXA` antes de mostrar qualquer dado — sem operador, só o card de login centralizado |
| `/cozinha` | `CozinhaHomePage` | **Real** — lista de pedidos e avanço de status (`ENVIADO_PARA_COZINHA`→`EM_PREPARO`→`PRONTO`) do dispositivo COZINHA. Desde a TASK-111, exige login de `OPERADOR_COZINHA` antes de mostrar qualquer dado — sem operador, só o card de login centralizado |
