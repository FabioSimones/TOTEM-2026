...
- **`imagemUrl` continua uma URL de texto livre** (`string`, opcional, máx. 500 caracteres) — a TASK-052 não implementou upload de arquivo real, apenas melhorou a experiência de digitar essa URL: validação básica de formato no cliente (precisa começar com `http://`/`https://` e ser aceita pelo construtor `URL` do navegador) e uma prévia da imagem exibida abaixo do campo, antes de salvar. Nenhum campo novo é enviado ao backend; o contrato continua exatamente `imagemUrl?: string`.
