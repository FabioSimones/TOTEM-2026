...
## Como testar refresh token e renovação automática de sessão (TASK-063)
...
5. Confira no DevTools → Network: deve aparecer a chamada original com `401`, seguida de `POST /api/auth/refresh` (200), seguida da chamada original repetida (200) — só 1 tentativa de refresh, nunca um loop.
...
7. Chamadas concorrentes (ex.: duas abas fazendo requisições ao mesmo tempo com o mesmo `accessToken` expirado) reaproveitam a mesma renovação em andamento — não disparam dois refreshes em paralelo (o refresh token é de uso único; duas renovações simultâneas desperdiçariam uma delas).
...
1. Login com um usuário `ADMIN_RESTAURANTE`: em `/admin`, os cards "Restaurantes" e "Usuários" não aparecem (continuam só para `SUPER_ADMIN` — o backend também bloqueia esses dois com `403`); aparece o aviso "Você está operando apenas no restaurante vinculado à sua conta.".
...
- ~~Sem refresh token nem logout no backend~~ **implementado na TASK-063** — "Sair" (`AdminHomePage`) agora chama `POST /api/auth/logout`, revogando o `refreshToken` no backend de verdade (best-effort: falha de rede ainda limpa a sessão local). O `accessToken` (JWT) em si continua sem revogação ativa — só expira por tempo (`app.security.jwt.expiration-minutes`); é o `refreshToken` que agora tem revogação real e é renovado automaticamente pelo `api.ts` em caso de `401`.
...
- ~~Sem seletor de restaurante ciente do escopo do `ADMIN_RESTAURANTE`~~ **resolvido na TASK-059** — ver seção "Como testar o escopo de ADMIN_RESTAURANTE" abaixo.
- **`OPERADOR_CAIXA`/`OPERADOR_COZINHA` no painel Admin** — a TASK-059 só ajustou a visibilidade de cards/formulários para `SUPER_ADMIN` vs. demais perfis (tratados como "restrito"); esses dois perfis nunca deveriam de fato acessar `/admin` (não têm nenhum uso real ali), mas nada no frontend impede o login — a proteção real é sempre o backend (403 em qualquer endpoint administrativo).
...
3. ~~Refresh token / expiração de sessão tratada~~ **implementado na TASK-063 para sessão de usuário administrativo** (login/refresh/logout, renovação automática em `api.ts`). Dispositivos (Totem/Caixa/Cozinha) continuam sem refresh token — token único de longa duração, sem renovação automática; permanece como possível task futura, se necessário.
