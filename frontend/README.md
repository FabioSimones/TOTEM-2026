...
## Como testar o upload de imagem de produto (TASK-053)

A partir da TASK-053, `/admin/produtos` permite enviar um arquivo de imagem de verdade (além do campo `imagemUrl` manual, que continua existindo). O backend salva o arquivo **localmente em disco** (`app.uploads.dir`, ver `docs/09-contratos-api.md`) — **essa é uma decisão de MVP**: em produção, isso deve ser substituído por um storage externo (S3, Cloudinary ou equivalente). Não há redimensionamento, compressão, upload múltiplo, galeria de mídia nem limpeza automática de arquivos antigos/órfãos.
