...
A partir da TASK-043, `/admin/dispositivos` é a primeira área administrativa real — permite listar, cadastrar, revogar e reativar dispositivos (Totem/Caixa/Cozinha/Administração). **Edição de dispositivo e paginação/filtros continuam fora do escopo**. A **TASK-050** trocou o campo numérico avulso "ID do restaurante" por um seletor visual (mesmo padrão de botões usado em Categorias/Produtos/Usuários) — o admin não digita mais o ID manualmente, escolhe o restaurante pelo nome.
...
15. Para simular erro 400, tente cadastrar um dispositivo com `codigoIdentificacao` já usado por outro — a mensagem de erro do backend aparece no formulário, sem perder os dados já digitados.
16. Para simular erro 404, cadastre um dispositivo, exclua o restaurante correspondente (se possível) ou desatualize a lista local e tente enviar novamente — mensagem "Restaurante não encontrado. Atualize a lista de restaurantes e tente novamente.".
17. Para simular sessão expirada, edite `totem.accessToken` no DevTools para um valor inválido e clique em "Atualizar lista" ou tente cadastrar: aparece mensagem de sessão expirada e o botão "Ir para login" (o formulário e a lista somem enquanto a sessão estiver inválida).
18. Alterne o tema (💡) com o formulário preenchido (seletor de restaurante e de tipo), com dispositivos ativos e revogados na lista — os badges de status ("Ativo" em verde, "Revogado" em vermelho, via `--color-success`/`--color-error`) e o bloco do código de ativação seguem os tokens do Design System nos dois temas.
...
1. Edição de dispositivo (`PUT /api/admin/dispositivos/{id}`, se/quando existir no backend) — hoje só criação, revogação e reativação são suportadas, conforme escopo da TASK-043.
2. Upload de imagem de produto — hoje `imagemUrl` é só um campo de texto (o admin cola uma URL já hospedada em outro lugar); um upload de verdade exigiria armazenamento de arquivo (S3, disco local, etc.), fora do escopo da TASK-046.
