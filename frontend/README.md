...
`/ativar-dispositivo` (TASK-029), `/totem` (TASK-031 a 035), `/caixa` (TASK-036, TASK-037, TASK-039 e TASK-040), `/cozinha` (TASK-038) e `/admin`+`/admin/login`+`/admin/dispositivos`+`/admin/restaurantes` (TASK-042 a TASK-044) têm lógica real. `HomePage` (`/`) continua sendo apenas o ponto de entrada, sem lógica própria.
...
1. CRUD administrativo de Categorias, Produtos e Usuários dentro de `/admin` — os cards "Em breve" em `AdminHomePage` já indicam onde cada área vai entrar; `/admin/dispositivos` e `/admin/restaurantes` (TASK-043/044) servem de referência de padrão (form único criar/editar + lista de cards + ações) para replicar nessas telas. Categorias/Produtos vão precisar de um seletor de restaurante — hoje só há um campo `ID do restaurante` avulso em Dispositivos; vale reaproveitar `adminRestauranteService.listarRestaurantes()` para um dropdown real quando essas telas existirem.
...
3. Proteção de rotas mais robusta (ex.: componente `ProtectedRoute` reutilizável, checagem de `perfil` por rota) — hoje cada página (`TotemHomePage`, `CaixaHomePage`, `CozinhaHomePage`, `AdminHomePage`, `AdminDispositivosPage`, `AdminRestaurantesPage`) repete a mesma checagem de sessão individualmente, e nenhuma delas bloqueia por `perfil` no frontend (a proteção real é sempre o backend retornando 403).
