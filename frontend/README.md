...
8. ~~Achado na TASK-060: editar `totem.accessToken` para um valor inválido não produzia `401`~~ **corrigido na TASK-061**: o backend agora responde `401` (via `RestAuthenticationEntryPoint`) para qualquer token ausente/inválido/expirado em endpoint protegido; `403` continua reservado a "autenticado mas sem permissão". Editar `totem.accessToken` no DevTools para um valor inválido e tentar qualquer ação agora mostra "Sessão expirada. Faça login novamente." e limpa a sessão corretamente, em vez de "Você não tem permissão...".
