...
3. **Decisão arquitetural: operador humano x dispositivo no fluxo Caixa/Cozinha** (diagnosticada na TASK-090-A, ainda em aberto após a TASK-090). A TASK-090 habilitou `ADMIN_RESTAURANTE` a cadastrar `OPERADOR_CAIXA`/`OPERADOR_COZINHA`, mas esses usuários continuam sem qualquer função no fluxo operacional real — `/api/caixa/**`/`/api/cozinha/**` seguem 100% dispositivo. Antes de investir em login de operador dentro do dispositivo e auditoria por usuário humano (`HistoricoStatusPedido.alteradoPorUsuario`, coluna já existente e nunca preenchida), vale decidir explicitamente se o modelo final é "só dispositivo" (e então considerar remover/deixar claro que os perfis de operador são só cadastrais) ou "dispositivo + operador" (e então planejar a task de login de operador, que exigiria resolver `restauranteId` do operador do mesmo jeito que `AdminScopeService` já resolve para o Admin).
