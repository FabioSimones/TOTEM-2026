...
**TASK-100 (correção de CI — falha nos jobs backend do GitHub Actions)**:
...
2. Considerar badge de status do CI no `README.md` e, se o time adotar branch protection no GitHub, exigir os 3 jobs de `ci.yml` como check obrigatório antes de merge em `main` (fora de escopo da TASK-084, que só criou o pipeline). **TASK-099 (observabilidade mínima) concluída** — ver `docs/testes-backend-mvp.md` seção 7-sedecies; próxima recomendada é TASK-100 (branch protection + badge de CI), primeiro item da tabela de priorização em `docs/roadmap-pos-mvp.md`.
