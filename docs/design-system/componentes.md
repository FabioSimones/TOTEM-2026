...
- **Coluna vs. conteúdo sticky (TASK-122)**: `AdminSidebar` agora renderiza um `<aside className="admin-sidebar-column">` envolvendo o `<nav className="admin-sidebar">` — mesmo padrão adotado no Totem (TASK-120.5), corrigindo o mesmo bug estrutural: `.admin-sidebar` acumulava item do flex row + contexto sticky (`height:100dvh`) + dono do fundo visual no mesmo elemento, então em páginas administrativas longas (`/admin/usuarios`, `/admin/produtos`, `/admin/pedidos` com muitos cards) o fundo/borda da sidebar terminava antes do fim da página. A **coluna** (`aside`) é o item real do flex row `.admin-layout` (sem `height` própria, estica via `align-items: stretch` do pai até a altura do irmão `.admin-layout__main`) e carrega fundo/borda/divisão. O **conteúdo interno** (`nav`, inalterado por dentro: marca + botão de recolher + lista de navegação) continua com `position: sticky; top: 0; height: 100dvh`, mas no desktop é transparente — quem pinta a coluna inteira, do início ao fim da página, é o `aside` por trás. Sidebar recolhida aplica a mesma lógica (`.admin-sidebar-column--collapsed`). No mobile, a coluna recolhe a zero dentro do `@media (max-width: 959px)` — o `nav` retoma fundo/borda próprios ali, pois ele mesmo vira o drawer (`position: fixed`), fora do fluxo da coluna. **Causa raiz nº2 removida nesta task**: `.admin-layout` tinha `overflow-x: hidden` sem `overflow-y` — pela spec do CSS isso força o outro eixo a computar como `auto`, transformando `.admin-layout` em seu próprio contêiner de scroll/clip e quebrando o `position: sticky` do `nav` assim que uma página administrativa passava de uma tela de altura; removido por já estar coberto globalmente por `html { overflow-x: hidden }` desde a TASK-120.5.
