...
- [x] **TASK-111**: `/caixa` e `/cozinha` recém-ativados mostram **somente** o card de login de operador centralizado — nenhum pedido é buscado nem exibido até o operador (`OPERADOR_CAIXA`/`OPERADOR_COZINHA` do mesmo restaurante) se identificar. O backend agora exige o header `X-Operador-Token` também nas listagens (`GET /api/caixa/pedidos/pendentes`, `GET /api/cozinha/pedidos`), não só nas ações de escrita — 401 sem operador, 403 se o operador for de outro tipo/restaurante.
- [x] **TASK-112**: sem dispositivo ativado, `/caixa`/`/cozinha` mostram um card centralizado ("Caixa/Cozinha não ativado(a)") com o botão "Ativar este dispositivo", em vez de redirecionar silenciosamente. Dispositivo de outro tipo (ex.: COZINHA abrindo `/caixa`) mostra "Dispositivo incompatível" citando o tipo atual, com botão "Trocar dispositivo" (pede confirmação, limpa dispositivo+operador, mantém a sessão administrativa intacta). Com dispositivo e operador identificados, "Trocar dispositivo" também fica disponível ao lado de "Trocar operador".
- [x] **TASK-119**: com dispositivo e operador prontos, `/caixa`/`/cozinha` passam a usar `OperationalLayout`/`OperationalTopbar` (não `AdminSidebar`) — módulo, nome/tipo do dispositivo, `ThemeToggle` e identidade do operador (avatar + nome + perfil amigável) ficam sempre visíveis no topo, nunca atrás de menu. "Trocar operador" e "Trocar dispositivo" continuam ações distintas (limpezas de sessão inalteradas, só a apresentação mudou) e ganharam ícones próprios (pessoa vs. dispositivo) — achado real de QA: antes usavam o mesmo ícone genérico de troca, indistinguíveis em mobile quando só o ícone fica visível. Hierarquia dos cards da Cozinha reordenada (número → tempo de espera → itens → observações → status → ação); tempo mostrado sem classificar como "atrasado"/"recente" (nenhuma regra desse tipo existe no backend). Nenhuma regra de autenticação/autorização, contrato ou status de pedido foi alterada.
- [x] **TASK-119.2**: corrige um bug real deixado pela TASK-119 — o intervalo "dispositivo ativado, operador ainda não identificado" (o que qualquer pessoa vê logo após ativar um Caixa/Cozinha) continuava preso na casca visual antiga, sem a `OperationalTopbar`. `operador`/`onTrocarOperador` passaram a ser opcionais em `OperationalLayout`/`OperationalTopbar` — a topbar (módulo, dispositivo, `ThemeToggle`, "Trocar dispositivo") já monta antes do login; sem operador, não mostra avatar/perfil/"Trocar operador". `CaixaHomePage`/`CozinhaHomePage` fundiram os branches "sem operador"/"com operador" num único `OperationalLayout`, que não desmonta durante a transição de login. `OperadorPainel` passou a ser o próprio conteúdo dessa tela (título/descrição/formulário), sem mais renderizar seu próprio "Trocar dispositivo" (elimina a duplicação com a topbar). Nenhuma mudança de autenticação, autorização, contrato ou `X-Operador-Token`.
...
- [ ] Caixa: identificar-se como `OPERADOR_CAIXA` no card de login (TASK-111) antes de qualquer lista aparecer
...
- [ ] Cozinha: identificar-se como `OPERADOR_COZINHA` no card de login (TASK-111) antes de qualquer lista aparecer
...
**TASK-105 (E2E integrado com backend real — Caixa/Cozinha/operador)**: fecha a lacuna deixada pela TASK-104. `frontend/e2e-integrado/caixa-cozinha-operador-real.spec.ts` cobre, com backend real e zero mocks, o restante deste checklist: dispositivos CAIXA e COZINHA reais, dois operadores reais (`OPERADOR_CAIXA`/`OPERADOR_COZINHA`) identificados via `OperadorPainel` em dois `BrowserContext` simulando dois terminais físicos, pedido avançando `PAGO` → `ENVIADO_PARA_COZINHA` → `EM_PREPARO` → `PRONTO` → `RETIRADO` inteiramente por clique real na UI, e **validação do histórico de auditoria** (`GET /api/admin/pedidos/{id}`, campos `alteradoPorUsuarioNome`/`alteradoPorDispositivoNome`) confirmando que cada transição foi atribuída ao operador e dispositivo corretos — a mesma regra crítica da TASK-092 (`HistoricoStatusPedido.alteradoPorUsuario`), agora validada de ponta a ponta contra backend real. Pedido é criado e pago via API do Totem (não pela UI, já coberta pelo spec da TASK-104), para manter o foco em Caixa/Cozinha. Validado localmente: passou de primeira, confirmado de forma independente consultando `GET /api/admin/pedidos?statusPedido=RETIRADO`. **Não substitui este checklist** (continua a referência manual/`curl`).

**TASK-106 (E2E integrado no CI)**: os dois specs acima (TASK-104 e TASK-105) passaram a rodar automaticamente no GitHub Actions — job `Frontend E2E Integrado (Backend real)` em `.github/workflows/ci.yml`, que sobe PostgreSQL (`services:`) e o backend real (`mvn spring-boot:run` em background) no próprio runner antes de rodar `npm run e2e:integrado`. Validado localmente reproduzindo o mesmo padrão do workflow (mesma sequência de subida via `nohup`/`disown` + espera por `/actuator/health`) antes do push. **Ainda não é check obrigatório** em branch protection — ver `docs/ci-branch-protection.md`.
