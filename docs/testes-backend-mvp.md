...
- **Descoberto na validação manual da TASK-078, não corrigido (fora do escopo de "gestão operacional de dispositivos")**: os campos `criadoEm`/`atualizadoEm` gerenciados pelo Hibernate (`@CreationTimestamp`/`@UpdateTimestamp`, presentes em toda entidade do projeto — `Restaurante`, `Categoria`, `Produto`, `Usuario`, `Pedido`, `Dispositivo`) são gravados no fuso horário padrão da JVM (`America/Sao_Paulo`, confirmado via `user.timezone` nos relatórios de teste — sem `-Duser.timezone=UTC` configurado em nenhum lugar do projeto), enquanto os campos manuais controlados por código (`Dispositivo.ultimoAcesso`/`ativadoEm`/`revogadoEm`, desde a correção de bug da TASK-077; e todas as comparações via `Clock.systemUTC()` em `PedidoExpiracaoService`/`LoginAttemptService`/`DashboardAdminService`) usam UTC. Na validação real desta task, um dispositivo criado e ativado com ~30 segundos de diferença mostrou `criadoEm=19:17:16` e `ativadoEm=22:17:47` — 3h de diferença aparente (o offset exato de `America/Sao_Paulo`), pela mistura dos dois relógios no mesmo registro. **Não afeta a correção do `statusOperacional`** (que só compara `ultimoAcesso` contra `Clock.systemUTC()`, ambos no mesmo fuso — verificado nos 3 status testados manualmente: `NUNCA_USADO`, `USADO_RECENTEMENTE`, `REVOGADO`), mas pode confundir um admin lendo `criadoEm`/`ativadoEm`/`atualizadoEm` lado a lado, e é a mesma raiz da limitação já documentada para `Pedido.criadoEm` no Dashboard/Expiração (que assumia — incorretamente — que `criadoEm` já estava em UTC). Corrigir isso de verdade exigiria uma decisão de arquitetura abrangendo todas as entidades do projeto (configurar `hibernate.jdbc.time_zone=UTC` globalmente, ou `-Duser.timezone=UTC` na JVM, ou trocar todos os `@CreationTimestamp`/`@UpdateTimestamp` por campos manuais via `Clock`) — fora do escopo de uma task de validação pontual. Recomenda-se uma task dedicada de padronização de fuso horário.
