...
| `security/RefreshTokenServiceTest` (novo, TASK-063) | Unitário puro (Mockito): `criarParaUsuario` revoga refresh tokens ativos existentes do usuário antes de emitir um novo, nunca persiste o valor bruto (só o hash); `validarERevogar`: aceita token válido e já o revoga (rotação), rejeita token inexistente/revogado/expirado/sem usuário; `revogar`: idempotente (token inexistente ou já revogado não causa erro nem `save` extra) |
