...
| `security/SecurityHttpStatusTest` (novo, TASK-061) | Único teste da suíte com `@SpringBootTest` + `@AutoConfigureMockMvc` (contexto real, H2 em memória) em vez de Mockito puro — exercita a cadeia completa de segurança: sem token → `401`; token malformado → `401`; header `Authorization` sem prefixo `Bearer` → `401`; token válido de `ADMIN_RESTAURANTE` batendo em endpoint `SUPER_ADMIN` (`/api/admin/usuarios`) → `403`; `GET /api/health` → `200`; `GET /uploads/produtos/<inexistente>` → `404` (não `401`/`403`) |
